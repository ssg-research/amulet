...
def main(args: argparse.Namespace) -> None:
    ...
    recon_test_loader = torch.utils.data.DataLoader(dataset=test_set, batch_size=1, shuffle=False)

    # Data Reconstruction attack on target model 
    log.info('Running Data Reconstruction Attack on target model')

    input_size = (1,) + tuple(test_set[0][0].shape)
