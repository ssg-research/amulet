...
def main(args: argparse.Namespace) -> None:
    ...
    logging.basicConfig(level=logging.INFO, 
                        filename=log_dir / 'outlier_removal_model_extraction.log', 
                        filemode="w")
    ...
    target_train_set, adv_train_set = torch.utils.data.random_split(data.train_set, [target_train_size, adv_train_size], generator=generator)
    ...
    target_model_path = models_path / 'targetForExtraction' / f'adv_train_fraction_{args.adv_train_fraction}'
    ...
    # With model extraction the dataset split is different, so it requires a separate directory
    defended_model_path = models_path / 'outlier_removal' / f'percent_{args.percent}' / 'defendedForExtraction' / f'adv_train_fraction_{args.adv_train_fraction}'
    ...
    if attack_model_filename.exists():
        ...
        log.info("Running Model Extraction attack on target model")
    ...
    attack_model_2_path = models_path / 'outlier_removal' / f'percent_{args.percent}' / 'model_extraction' / f'adv_train_fraction_{args.adv_train_fraction}'
    ...
    if attack_model_2_filename.exists():
        ...
        log.info("Running Model Extraction attack on defended model")
