...
from amulet.risks import AttributeInference
...
def main(args: argparse.Namespace) -> None:
    ...
    logging.basicConfig(level=logging.INFO, 
                        filename=log_dir / 'adversarial_training_discriminatory_behavior.log', 
                        filemode="w")
    ...
    x_adv_train, x_test, y_adv_train, y_test, z_adv_train, z_test = train_test_split(x_test,y_test,z_test, test_size=0.5, random_state=42)
    ...
    attribute_inference = AttributeInference(target_model, x_adv_train, x_test, z_adv_train, z_test, args.device)
    mertrics = attribute_inference.attack_predictions()

    log.info("Attribute inference attack success on target model")
    accuracy_dict = {
        'race_attacc': mertrics[0][0],
        'race_auc': mertrics[0][1],
        'sex_attacc': mertrics[1][0],
        'sex_auc': mertrics[1][1]
    }
    log.info(accuracy_dict)

    # Evaluate discriminatory behavior on defended model 
    attribute_inference = AttributeInference(defended_model, x_adv_train, x_test, z_adv_train, z_test, args.device)
    mertrics = attribute_inference.attack_predictions()
    ...
    log.info("Attribute inference attack success on defended model")
    log.info(accuracy_dict)
