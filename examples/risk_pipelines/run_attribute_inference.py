...
def main(args: argparse.Namespace) -> None:
    ...
    logging.basicConfig(level=logging.INFO, 
                        filename=log_dir / 'model_extraction.log', 
                        filemode="w")
    ...
    split_data = train_test_split(x_train, y_train, z_train, test_size=args.adv_train_fraction)

    x_train_target, x_train_adv, y_train_target, y_train_adv, z_train_target, z_train_adv = split_data

    # Create data loaders
    target_train_set = torch.utils.data.TensorDataset(torch.from_numpy(x_train_target).type(torch.FloatTensor),
                                                      torch.from_numpy(y_train_target).type(torch.LongTensor))
    adv_train_set = torch.utils.data.TensorDataset(torch.from_numpy(x_train_adv).type(torch.FloatTensor),
                                                   torch.from_numpy(y_train_adv).type(torch.LongTensor))
    ...
    target_model_path = models_path / 'targetLimitedData' / f'adv_train_fraction_{args.adv_train_fraction}'
    ...
    attribute_inference = DudduCIKM2022(target_model, x_train_adv, x_test, z_train_adv, args.device)
