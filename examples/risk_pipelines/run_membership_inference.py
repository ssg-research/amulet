...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--pkeep', type = int, default = 0.5, help = 'Proportion of data to keep for training.')
    parser.add_argument('--num_shadow', type = int, default = 8, help = 'Number of shadow models to train.')
    parser.add_argument('--num_aug', type = int, default = 10, help = 'Number of shadow models to train.')
...
def main(args: argparse.Namespace) -> None:
    ...
    keep = np.random.choice(len(data.train_set), size=int(args.pkeep * len(data.train_set)), replace=False)
    keep.sort()
    target_train_set = torch.utils.data.Subset(data.train_set, keep)
    train_loader = torch.utils.data.DataLoader(dataset=target_train_set, batch_size=args.batch_size, shuffle=False)
