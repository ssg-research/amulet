...
def main(args: argparse.Namespace) -> None:
    ...
    logging.basicConfig(level=logging.INFO, 
                        filename=log_dir / 'model_poisoning.log', 
                        filemode="w")
    ...
    target_train_loader = torch.utils.data.DataLoader(dataset=data.train_set, batch_size=args.batch_size, shuffle=False)
    ...
    poisoned_model_path = models_path / 'model_poisoning' / f'poisoned_protion_{args.poisoned_portion}'
    ...
    if poisoned_model_filename.exists():
        ...
        poisoned_model = initialize_model(args.model, args.model_capacity, args.dataset, log).to(args.device)
        ...
        poisoned_train_loader = torch.utils.data.DataLoader(dataset=poisoned_train_set, batch_size=args.batch_size, shuffle=False)
        
        poisoned_model = train_classifier(poisoned_model, poisoned_train_loader, criterion, optimizer, args.epochs, args.device)

        poisoned_test_set = poisoning.poison_dataset(data.test_set, mode='test')
        poisoned_test_loader = torch.utils.data.DataLoader(dataset=poisoned_test_set, batch_size=args.batch_size, shuffle=False)

        # Save model
        create_dir(poisoned_model_path, log)
        torch.save(poisoned_model, poisoned_model_filename)
    
    log.info("Target Model on Origin Data: %s", get_accuracy(target_model, test_loader, args.device))
    log.info("Target Model on Poisoned Data: %s", get_accuracy(target_model, poisoned_test_loader, args.device))
    log.info("Poisoned Model on Origin Data: %s", get_accuracy(poisoned_model, test_loader, args.device))
    log.info("Poisoned Model on Poisoned Data: %s", get_accuracy(poisoned_model, poisoned_test_loader, args.device))
