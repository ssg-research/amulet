...
def main(args: argparse.Namespace) -> None:
    ...
    logging.basicConfig(level=logging.INFO, 
                        filename=log_dir / 'data_reconstruction.log', 
                        filemode='w')
    ...
    train_loader = torch.utils.data.DataLoader(dataset=data.train_set, batch_size=1, shuffle=False)
    test_loader = torch.utils.data.DataLoader(dataset=data.test_set, batch_size=1, shuffle=False)
    ...
    data_recon = FredriksonCCS2015(target_model,
                                   input_size,
                                   output_size,
                                   args.device,
                                   args.alpha)
    ...
    mse_loss = reverse_mse(test_loader, reverse_data, input_size, output_size, args.device)
