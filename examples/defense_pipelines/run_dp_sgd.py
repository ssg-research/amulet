...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--model', type = str, default = 'cnn', help = 'Options: vgg, linearnet, binarynet.')
    ...
    parser.add_argument("-c","--max-per-sample-grad_norm",type=float,default=1.0,metavar="C",help="Clip per-sample gradients to this norm")
    ...
    parser.add_argument("--secure_rng",type=bool, default=False,help="Enable Secure RNG to have trustworthy privacy guarantees. Comes at a performance cost")
...
def main(args: argparse.Namespace) -> None:
    ...
    logging.basicConfig(level=logging.INFO, 
                        filename=log_dir / 'dp_training.log', 
                        filemode="w")
    ...
    if defended_model_filename.exists():
        ...
        optimizer = torch.optim.Adam(target_model.parameters(), lr=1e-3)
        dp_training = DPSGD(target_model,
                            criterion,
                            optimizer,
                            train_loader,
                            args.device,
