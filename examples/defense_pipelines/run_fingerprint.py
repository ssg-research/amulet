...
import torch
from amulet.defenses import Fingerprinting
...
def main(args: argparse.Namespace) -> None:
    ...
    logging.basicConfig(level=logging.INFO, 
                        filename=log_dir / 'fingerprinting.log', 
                        filemode="w")
    ...
    if suspect_model_filename.exists():
        ...
        suspect_model = initialize_model(args.model, args.model_capacity, args.dataset, log).to(args.device)
        optimizer = torch.optim.Adam(suspect_model.parameters(), lr=1e-3)
        suspect_model = train_classifier(suspect_model, train_loader, criterion, optimizer, args.epochs, args.device)
        log.info("Target model trained")
    ...
    fingerprinting = Fingerprinting(target_model,
                                  suspect_model,
                                  train_loader,
                                  test_loader,
