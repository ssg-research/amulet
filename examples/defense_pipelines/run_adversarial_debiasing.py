...
def main(args: argparse.Namespace) -> None:
    ...
    sensitive_train_set = torch.utils.data.TensorDataset(torch.from_numpy(x_train).type(torch.FloatTensor), 
                                                         torch.from_numpy(y_train).type(torch.LongTensor), 
    ...
    sensitive_train_loader = torch.utils.data.DataLoader(dataset=sensitive_train_set, batch_size=args.batch_size, shuffle=False)
    ...
    discr_behavior_target = DiscriminatoryBehavior(target_model, sensitive_test_loader, args.device)
    ...
    if defended_model_filename.exists():
        ...
        group_fairness = AdversarialDebiasing(target_model,
                                              criterion,
                                              optimizer,
    ...
    discr_behavior_defended = DiscriminatoryBehavior(defended_model, sensitive_test_loader, args.device)
