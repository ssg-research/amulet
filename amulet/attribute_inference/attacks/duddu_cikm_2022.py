...
class DudduCIKM2022(AttributeInferenceAttack):
    ...
    def attack_predictions(self) -> Dict[int, Dict[str, np.ndarray]]:
        ...
        """
        predictions_train = self.model(torch.from_numpy(self.x_train_adv).type(torch.FloatTensor).to(self.device))
        predictions_train = predictions_train.detach().cpu().numpy()
        attack_model_train_x = pd.DataFrame(predictions_train, columns = ['class1','class2'])

        predictions_test = self.model(torch.from_numpy(self.x_test).type(torch.FloatTensor).to(self.device))
        predictions_test = predictions_test.detach().cpu().numpy()
        attack_model_test_x = pd.DataFrame(predictions_test, columns = ['class1','class2'])
        ...
        for i in range(num_sensitive_attributes):
            ...
            attack_model = MLPClassifier(solver='adam', alpha=1e-3, hidden_layer_sizes=(64,128,32,), verbose=0, max_iter=300,random_state=1337)
