...
class OutlierRemoval(PoisoningDefense):
    ...
    def _knn_shapely(self,
                     train_features: torch.Tensor,
                     train_targets: torch.Tensor,
                     test_features: torch.Tensor,
                     test_targets: torch.Tensor, k=5):
    ...
    def train_model(self,
                    get_hidden: Callable[[torch.nn.Module,
                                          torch.utils.data.DataLoader,
                                          torch.nn.Module,
                                          str],
                                         Tuple[torch.Tensor,
                                               torch.Tensor,
                                               torch.Tensor]] = get_intermediate_features
    ):
        """
        ...
        train_features, train_targets, train_inputs = get_hidden(self.model, self.train_loader, self.device)
        test_features, test_targets, _ = get_hidden(self.model, self.test_loader, self.device)

        # Calculate shapely scores for the data points
        shapley_scores = self._knn_shapely(train_features, train_targets, test_features, test_targets, k=5)
        normalized_scores = (shapley_scores-min(shapley_scores))/(max(shapley_scores)-min(shapley_scores))
        df = pd.Series({'Scores': np.array(normalized_scores),'train_inputs': train_inputs, 'train_targets': train_targets})
        ...
        train_data_new = torch.utils.data.TensorDataset(torch.from_numpy(np.array(train_inputs_new)).type(torch.FloatTensor),
                                                        torch.from_numpy(np.array(train_targets_new)).type(torch.LongTensor))
        train_loader_new = torch.utils.data.DataLoader(dataset=train_data_new, batch_size=self.batch_size, shuffle=False)

        # Retrain model with outliers removed
        criterion = torch.nn.CrossEntropyLoss()
        optimizer = torch.optim.Adam(self.model.parameters(), lr=1e-3)
        self.model = self.train(self.model, train_loader_new, criterion, optimizer, self.epochs, self.device)
