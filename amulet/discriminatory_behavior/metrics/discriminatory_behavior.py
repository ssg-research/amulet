...
class DiscriminatoryBehavior():
    ...
    @staticmethod
    def accuracy(predictions: np.ndarray, targets: np.ndarray, attributes: np.ndarray) -> Tuple[float, float]:
    ...
    @staticmethod
    def true_positive_parity(predictions: np.ndarray, targets: np.ndarray, attributes: np.ndarray) -> float:
        """
    ...
    @staticmethod
    def false_positive_parity(predictions: np.ndarray, targets: np.ndarray, attributes: np.ndarray) -> float:
        """
    ...
    def evaluate_subgroup_metrics(self):
        ...
        with torch.no_grad():     
            for data, target, sensitive_attributes in self.test_loader:
                data, target, sensitive_attributes = data.to(self.device), target.to(self.device), sensitive_attributes.to(self.device)
        ...
        for i in range(num_attributes):
            ...
            metrics[i]['acc_true'], metrics[i]['acc_false'] = self.accuracy(predictions, targets, attribute_arr)
            metrics[i]['demographic_parity'] = self.demographic_parity(predictions, attribute_arr)
            metrics[i]['true_positive_parity'] = self.true_positive_parity(predictions, targets, attribute_arr)
            metrics[i]['false_positive_parity'] = self.false_positive_parity(predictions, targets, attribute_arr)
            metrics[i]['p_rule'] = self.p_rule(predictions, attribute_arr)
            metrics[i]['equalized_odds'] = metrics[i]['true_positive_parity'] + metrics[i]['false_positive_parity']
