...
class EvasionPGD(EvasionAttack):
    ...
    def __init__(
            ...
            device: str,
            batch_size: int,
            epsilon: Optional[int] = 16,
            iterations: Optional[int] = 40,
            step_size: Optional[float] = 0.02
    ):
    ...
    def run_evasion(self) -> float:
        ...
        for x, y in self.test_loader:
            ...
            x_pgd = projected_gradient_descent(self.model,
                                               x,
                                               self.epsilon/255,
                                               self.step_size,
                                               self.iterations,
        ...
        adversarial_test_set = torch.utils.data.TensorDataset(torch.from_numpy(adv_input).type(torch.FloatTensor),
                                                              torch.from_numpy(labels).type(torch.LongTensor))
        advesrarial_test_loader = torch.utils.data.DataLoader(dataset=adversarial_test_set, batch_size=self.batch_size, shuffle=False)
