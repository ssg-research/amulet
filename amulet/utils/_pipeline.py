...
def load_data(root: Path | str,
    ...
    if dataset == 'cifar10':
        ...
        data = load_census(root / 'data' / 'census', random_seed = exp_id, return_x_y_z = return_x_y_z)
    elif dataset == 'lfw':
        data = load_lfw(root / 'data' / 'lfw', random_seed = exp_id, return_x_y_z=return_x_y_z)
    else:
        if log:
            log.info('Line 42, mlconf.utils._pipeline.py: Incorrect dataset configuration')
        else:
    ...
    if return_x_y_z:
        ...
        if training_size < 1.0:
            x_train, _, y_train, _, z_train, _ = train_test_split(x_train, 
                                                                  y_train, 
                                                                  z_train, 
                                                                  train_size = training_size,
                                                                  random_state = exp_id)
            
        data = (np.array(x_train), np.array(x_test), 
                np.array(y_train), np.array(y_test), 
                np.array(z_train), np.array(z_test))
    else:
        new_train_size = int(training_size*len(data.train_set))
        train_set, _ = torch.utils.data.random_split(data.train_set, [new_train_size, len(data.train_set) - new_train_size], generator=generator)
        data.train_set = train_set

    if log:
        log.info('Dataset loaded: %s', dataset)
    else:
...
def initialize_model(model: str,
    ...
    if model == 'vgg':
        ...
        model = CNN()
    elif model == 'linearnet':
        model = LinearNet(hidden_layer_sizes=capacity_map[model_capacity]['linearnet'])
    elif model == 'binarynet':
        if dataset == 'census':
            model = BinaryNet(num_features=93, hidden_layer_sizes=capacity_map[model_capacity]['binarynet'])
        elif dataset == 'lfw':
            model = BinaryNet(num_features=8742, hidden_layer_sizes=capacity_map[model_capacity]['binarynet'])
        else:
            if log:
                log.info('Line 148, mlconf.utils._pipeline.py: Incorrect model configuration.')
            else:
                print('Line 150, mlconf.utils._pipeline.py: Incorrect model configuration.')
            sys.exit()
    else:
        if log:
            log.info('Line 154, mlconf.utils._pipeline.py: Incorrect model configuration')
        else:
