...
def get_filter(df, filter_prop, split, ratio, dataset_name, is_test):
    ...
    if dataset_name == "census":
        if filter_prop == "sex":
            ...
            def lambda_fn(x): return x['race'] == 1       
        ...
        return heuristic(df, lambda_fn, ratio,subsample_size, class_imbalance=3,n_tries=100, class_col='y',verbose=False)
...
class DistributionInference():
    ...
    def prepare_dataset(self):
        ...
        y_train = pd.DataFrame(self.y_train, columns=["y"])
        z_train = pd.DataFrame(self.z_train, columns=['race', 'sex'])
        df_train = pd.concat([x_train, y_train, z_train], axis=1)
        x_test = pd.DataFrame(self.x_test)
        y_test = pd.DataFrame(self.y_test, columns=["y"])
        z_test = pd.DataFrame(self.z_test, columns=['race', 'sex'])
        ...
        def prepare_one_set(TRAIN_DF, TEST_DF, split, prop_ratio, filter_prop):
            TRAIN_DF = get_filter(TRAIN_DF, filter_prop,split, prop_ratio, self.args.dataset, is_test=0)
            TEST_DF = get_filter(TEST_DF, filter_prop,split, prop_ratio, self.args.dataset, is_test=1) # keep the test dataset fixed
            (x_tr, y_tr, cols), (x_te, y_te, cols) = self.get_x_y(TRAIN_DF), self.get_x_y(TEST_DF)
            return (x_tr, y_tr), (x_te, y_te), cols

        (X_train_victim_1, y_train_victim_1), (X_test_victim_1, y_test_victim_1), _ =  prepare_one_set(self.train_df_victim, self.test_df_victim, "victim", self.ratio1, self.filter_prop)
        (X_train_attacker_1, y_train_attacker_1), (X_test_attacker_1, y_test_attacker_1), _ =  prepare_one_set(self.train_df_adv, self.test_df_adv, "attacker", self.ratio1, self.filter_prop)
        (X_train_victim_2, y_train_victim_2), (X_test_victim_2, y_test_victim_2), _ =  prepare_one_set(self.train_df_victim, self.test_df_victim, "victim", self.ratio2, self.filter_prop)
        (X_train_attacker_2, y_train_attacker_2), (X_test_attacker_2, y_test_attacker_2), _ =  prepare_one_set(self.train_df_adv, self.test_df_adv, "attacker", self.ratio2, self.filter_prop)

        self.vic_traindata_1 = Data.TensorDataset(torch.from_numpy(np.array(X_train_victim_1)).type(torch.FloatTensor), torch.from_numpy(np.array(y_train_victim_1)).type(torch.LongTensor).squeeze(1))
        self.att_traindata_1 = Data.TensorDataset(torch.from_numpy(np.array(X_train_attacker_1)).type(torch.FloatTensor), torch.from_numpy(np.array(y_train_attacker_1)).type(torch.LongTensor).squeeze(1))
        self.vic_traindata_2 = Data.TensorDataset(torch.from_numpy(np.array(X_train_victim_2)).type(torch.FloatTensor), torch.from_numpy(np.array(y_train_victim_2)).type(torch.LongTensor).squeeze(1))
        self.att_traindata_2 = Data.TensorDataset(torch.from_numpy(np.array(X_train_attacker_2)).type(torch.FloatTensor), torch.from_numpy(np.array(y_train_attacker_2)).type(torch.LongTensor).squeeze(1))

        self.vic_testdata_1 = Data.TensorDataset(torch.from_numpy(np.array(X_test_victim_1)).type(torch.FloatTensor), torch.from_numpy(np.array(y_test_victim_1)).type(torch.LongTensor).squeeze(1))
        self.att_testdata_1 = Data.TensorDataset(torch.from_numpy(np.array(X_test_attacker_1)).type(torch.FloatTensor), torch.from_numpy(np.array(y_test_attacker_1)).type(torch.LongTensor).squeeze(1))
        self.vic_testdata_2 = Data.TensorDataset(torch.from_numpy(np.array(X_test_victim_2)).type(torch.FloatTensor), torch.from_numpy(np.array(y_test_victim_2)).type(torch.LongTensor).squeeze(1))
        self.att_testdata_2 = Data.TensorDataset(torch.from_numpy(np.array(X_test_attacker_2)).type(torch.FloatTensor), torch.from_numpy(np.array(y_test_attacker_2)).type(torch.LongTensor).squeeze(1))

        testdata_1 = torch.utils.data.ConcatDataset([self.att_testdata_1, self.vic_testdata_1])
        testdata_2 = torch.utils.data.ConcatDataset([self.att_testdata_2, self.vic_testdata_2])

        vic_trainloader_1 = torch.utils.data.DataLoader(dataset=self.vic_traindata_1, batch_size=256, shuffle=False)
        vic_trainloader_2 = torch.utils.data.DataLoader(dataset=self.vic_traindata_2, batch_size=256, shuffle=False)
        att_trainloader_1 = torch.utils.data.DataLoader(dataset=self.att_traindata_1, batch_size=256, shuffle=False)
        att_trainloader_2 = torch.utils.data.DataLoader(dataset=self.att_traindata_2, batch_size=256, shuffle=False)
        test_loader_1 = torch.utils.data.DataLoader(dataset=testdata_1, batch_size=256, shuffle=False) 
        test_loader_2 = torch.utils.data.DataLoader(dataset=testdata_2, batch_size=256, shuffle=False) 

        return vic_trainloader_1, vic_trainloader_2, att_trainloader_1, att_trainloader_2, test_loader_1, test_loader_2
    ...
    def get_preds(self, loader, models):
        ...
        for data in loader:
            if len(data) == 2:
                ...
                features, labels, _ =  data[0], data[1], data[2]
    ...
    def attack(self, models_vic_1, models_vic_2, models_adv_1, models_adv_2, testloader_1, testloader_2):
        ...
        preds_1_first, preds_1_second = self._get_kl_preds(preds_adv_prop1_dist1,preds_adv_prop2_dist1,preds_vic_prop1_dist1,preds_vic_prop2_dist1)
        preds_2_first, preds_2_second = self._get_kl_preds(preds_adv_prop1_dist2,preds_adv_prop2_dist2,preds_vic_prop1_dist2,preds_vic_prop2_dist2)
