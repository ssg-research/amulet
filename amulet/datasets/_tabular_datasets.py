...
def load_census(path: Union[str, Path] = Path('./data/census'), 
                random_seed: int = 7,
    ...
    if not os.path.exists(filename):
        ...
        adult_data = fetch_ucirepo(id=2).data.original[column_names]
        adult_data = (adult_data.replace('?', np.NaN)
                      .loc[lambda df: df['race']
                           .isin(['White', 'Black'])])
        adult_data.to_csv(filename, index=False)
    else:
        adult_data = pd.read_csv(filename, dtype=dtypes)
    ...
    if return_x_y_z:
        return np.array(x_train), np.array(x_test), y_train, y_test, z_train, z_test
...
def load_lfw(path: Union[str, Path] = Path('./data/lfw'),
             target: str = 'age',
    ...
    if not os.path.exists(images_path):
        ...
        with urlopen('https://www.cs.columbia.edu/CAVE/databases/pubfig/download/lfw_attributes.txt') as response:
        ...
        for name, num in zip(names, img_num):
            name = name.replace(' ', '_')
            img_path =  path / 'lfw_home' / 'lfw_funneled' / name / f'{name}_{str(num).zfill(4)}.jpg'
    ...
    def _load_lfw_attr(attribute: str):
        return _load_lfw_binary_attr(attribute) if attribute in BINARY_ATTRS else _load_lfw_multi_attr(attribute)
    ...
    target_array = np.asarray([target_labels[i] for i in common_indices], dtype=np.int32)
    sensitive_attrs_1 = np.asarray([sensitive_attr_1[i] for i in common_indices], dtype=np.int32)
    sensitive_attrs_2 = np.asarray([sensitive_attr_2[i] for i in common_indices], dtype=np.int32)
    ...
    # Split data into train / test
    x_train, x_test, y_train, y_test, z_train, z_test = train_test_split(x, y, z, test_size = 0.3, random_state = random_seed)
    ...
    # Create datasets
    train_set = torch.utils.data.TensorDataset(torch.from_numpy(np.array(x_train)).type(torch.FloatTensor),
                                                torch.from_numpy(np.array(y_train)).type(torch.LongTensor).squeeze(1))
    test_set = torch.utils.data.TensorDataset(torch.from_numpy(np.array(x_test)).type(torch.FloatTensor),
                                               torch.from_numpy(np.array(y_test)).type(torch.LongTensor).squeeze(1))
