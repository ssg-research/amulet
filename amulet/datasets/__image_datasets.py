...
def load_celeba(
    ...
    df = pd.read_csv(path / 'celeba.csv', na_values="NA", index_col=None, sep=",", header=0)
    ...
    attributes = df[['Smiling', 'Male']]

    images_train, images_test, attributes_train, attributes_test = train_test_split(images_np, attributes, test_size = test_size, stratify = attributes, random_state = random_seed)

    target_train = attributes_train['Smiling'].to_numpy()
    target_test = attributes_test['Smiling'].to_numpy()
    sensitive_train = attributes_train[['Male']].to_numpy()
    sensitive_test = attributes_test[['Male']].to_numpy()

    train_set = TensorDataset(torch.from_numpy(images_train).type(torch.FloatTensor), torch.from_numpy(target_train).type(torch.LongTensor))
    test_set = TensorDataset(torch.from_numpy(images_test).type(torch.FloatTensor), torch.from_numpy(target_test).type(torch.LongTensor))
