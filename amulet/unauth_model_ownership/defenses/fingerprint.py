...
class Fingerprinting():
    ...
    def __init__(
            ...
            alpha_l1: Optional[float] = 1.0,
            alpha_l2: Optional[float] = 0.01,
            alpha_linf: Optional[float] = 0.001,
            k: Optional[float] = 1.0,
            gap: Optional[float] = 0.001,
    ...
    def dataset_inference(self):
        ...
        for name in names:
            ...
            _, pval = ttest_ind(pred_test, pred_train, alternative="greater", equal_var=False)
            if pval < 0:
    ...
    def _get_mingd_vulnerability(self, loader, model, num_images = 100):
        ...
        for i,batch in enumerate(loader):
            if self.regressor_embed == 1: ##We need an extra set of `distinct images for training the confidence regressor
            ...
            for j, distance in enumerate(["linf", "l2", "l1"]):
                ...
                for target_i in range(self.num_classes):
                    ...
                    yp = model(X+delta) 
                    distance_dict = {"linf": self._norms_linf_squeezed, "l1": self._norms_l1_squeezed, "l2": self._norms_l2_squeezed}
    ...
    def _mingd(self, model, X, target):
        ...
        alpha_map = {"l1": self.alpha_l1/self.k, "l2": self.alpha_l2, "linf": self.alpha_linf}
        ...
        for t in range(self.num_iter):        
            ...
            if self.distance == "linf":
                ...
                if self.dataset == "1D":
                    delta_r.data += alpha * (grads / torch.norm(grads+1e-12,dim=1).unsqueeze(1))
                else:
                    delta_r.data += alpha * (grads / self._norms_l2(grads + 1e-12))
            elif self.distance == "l1":
                if self.dataset == "1D":
                    delta_r.data += alpha * self._l1_dir_topk_size2(grads, delta_r.data, X_r, self.gap, self.k)
                else:
                    delta_r.data += alpha * self._l1_dir_topk(grads, delta_r.data, X_r, self.gap, self.k)
    ...
    def _l1_dir_topk(self, grad, delta, X, gap, k = 10) :
    ...
    def _l1_dir_topk_size2(self, grad, delta, X, gap, k = 10):
        #Check which all directions can still be increased such that
        #they haven't been clipped already and have scope of increasing
        # ipdb.set_trace()
        X_curr = X + delta
