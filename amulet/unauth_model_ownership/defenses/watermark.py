...
class WatermarkNN():
    ...
    def __init__(
            ...
            device: str,
            wm_path = str,
            gray : Optional[bool] = 0,
            tabular: Optional[bool] = 0,
        ...
        shape = shape = next(iter(train_loader))[0].shape[-1]
    ...
    def get_wm_loader(
        ...
        if tabular:
            ...
            wm_dataset = data.TensorDataset(torch.from_numpy(np.array(wm_data)).type(torch.FloatTensor), torch.from_numpy(np.array(wm_label)).type(torch.LongTensor))
            wm_loader = DataLoader(dataset=wm_dataset, batch_size=self.batch_size, shuffle=False)
        else:
            ...
            wm_loader = torch.utils.data.DataLoader(
                wmset, batch_size=self.batch_size, shuffle=True,
    ...
    def watermark(self):
        ...
        for epoch in range(self.epochs):
            ...
            for batch_idx, (tuple) in enumerate(self.train_loader):
                ...
                target = torch.cat([target, wmtargets[(wm_idx + batch_idx) % len(wminputs)]], dim=0)
